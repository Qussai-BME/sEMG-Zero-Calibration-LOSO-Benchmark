"""
01c_correct_friedman_test.py — Corrects Friedman test p-values using existing Day 1 results.
Reads from BOTH JSON files AND generated CSV tables.
Usage: python 01c_correct_friedman_test.py
"""
import os
import sys
import json
...
def try_read_from_csv(results_dir):
    """
    Try reading per-subject accuracy from CSV files generated by 02e_generate_posthoc_figures_and_tables.py.
...
def main():
    ...
    if not any(day1[db] for db in ['DB7', 'DB3', 'DB2']):
        ...
        print('  python -c "import json; d=json.load(open(\'results/Ninapro_DB7_xgboost_results.json\')); [print(k, type(v).__name__, len(v) if hasattr(v,\'__len__\') else \'\') for k,v in d.items()]"')
        return
